...
//存入直方图像素点
unsigned char * HistogramEqualize::saveHistogram512_2(unsigned char * bmp, int width, int height, int linebyte, int num)
{
	int i, j;
	unsigned char * pBmpBuf = new unsigned char[512 * 512];
	int lineByte = (512 * 8 / 8 + 3) / 4 * 4;
...
//存入直方图像素点
unsigned char * HistogramEqualize::saveHistogram512_3(unsigned char * bmp, int width, int height, int linebyte, int num)
{
	int i, j;
	unsigned char * pBmpBuf = new unsigned char[512 * 512];
	int lineByte = (512 * 8 / 8 + 3) / 4 * 4;
	...
	for (i = 0; i < 512; i = i + 2) {
		...
		for (j = 0; j < 512; j++) {
			if (i == num * 2) {
				if (key) {
					pBmpBuf[j * lineByte + i] = 150;
					pBmpBuf[j * lineByte + i + 1] = 150;
					key = false;
				} else {
					key = true;
				}
