...
unsigned char * spatialFiltering::average_with_border(unsigned char * bmp, int width, int height, int lineByte)
{
...
unsigned char * spatialFiltering::median_with_border(unsigned char * bmp, int width, int height, int lineByte)
{
	int lineByteT = ((width + 2) * 8 / 8 + 3) / 4 * 4;
	pBmpBuf = new unsigned char[lineByteT*(height + 2)];
	for (i = 0; i < height + 2; i++)
